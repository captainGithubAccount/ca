...
public class MPChartUtil {
    ...
    static ArrayList<String> chartColors = new ArrayList<>();

    static {
        chartColors.add("#1F54FC");
        chartColors.add("#FFC64E");
        chartColors.add("#FF0A0C");
        chartColors.add("#91FC26");
        chartColors.add("#2CFEFE");
        chartColors.add("#FD890A");
        chartColors.add("#CE24A9");
        chartColors.add("#6B15C7");
        chartColors.add("#00A459");
        chartColors.add("#F9756A");
    }
...
//                if(time.length() >=6) {
//                    String temp = time.substring(0,6);
//                    StringBuilder strBuilder = new StringBuilder();
//                    strBuilder.append(temp.substring(0,2));
//                    strBuilder.append(":");
//                    strBuilder.append(temp.substring(2,4));
//                    strBuilder.append(":");
//                    strBuilder.append(temp.substring(4,6));
...
//                    return "" + (int) value;
        ...
        chart.invalidate();
        setChartData(chart, values, LineDataSet.Mode.CUBIC_BEZIER,0);
    }
     ...
     * @param values xy数据集合
     */
    public static void setChartData(LineChart chart, List<Entry> values, LineDataSet.Mode mode,int matrixId) {
        Log.i("capture","设置图表" + matrixId);
        LineDataSet lineDataSet;
        int index = matrixId -1;
        //getDataSetCount() 总线条数,getDataSetByIndex(0) 索引0处的xy数据的集合（30天就是30个 0-29）
        if (chart.getData() != null && chart.getData().getDataSetCount() > 0 && index < chart.getData().getDataSetCount()) {
            Log.i("capture","图 " + matrixId + "存在");
            lineDataSet = (LineDataSet) chart.getData().getDataSetByIndex(index);
            lineDataSet.setValues(values);
            chart.getData().notifyDataChanged();
            chart.notifyDataSetChanged();
        } else {
            Log.i("capture","图: " + matrixId + "初始化data");
            //初始化折线
            lineDataSet = new LineDataSet(values, "zone" + matrixId);
            // 设置曲线颜色
            lineDataSet.setColor(Color.parseColor(chartColors.get(matrixId-1)));
